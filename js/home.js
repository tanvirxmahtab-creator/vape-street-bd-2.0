...
function heroVideo(){const video=document.querySelector(".hero__video");if(!video)return;if(window.IS_LOW_END_DEVICE||document.documentElement?.classList.contains("low-power-mode")||matchMedia("(prefers-reduced-motion:reduce)").matches){video.pause();video.style.display="none";return;}const play=()=>video.play().catch(()=>{});video.loop=false;video.addEventListener("canplay",play,{once:true});if(video.readyState>=3)play();video.addEventListener("ended",()=>video.pause())}
